...
# Fixed lock stripes bound memory while serializing pending-command changes
# for each conversation within this process.
_pending_locks = tuple(threading.RLock() for _ in range(256))


def _pending_lock(conversation_id: str) -> threading.RLock:
    return _pending_locks[hash(conversation_id) % len(_pending_locks)]
...
def _queue_pending(conversation_id, command, args, summary) -> str:
    pid = new_id()
    with _pending_lock(conversation_id):
        # A fresh plan supersedes an earlier unconfirmed plan.
        execute("DELETE FROM pending_commands WHERE conversation_id = ?",
                (conversation_id,))
...
def _cmd_create_gateway(project_id, args, ts) -> str:
...
def _cmd_test_gateway(project_id, args, ts) -> str:
...
def _cmd_create_role(project_id, args, ts) -> str:
...
def _cmd_create_persona(project_id, args, ts) -> str:
...
def _cmd_create_skill(project_id, args, ts) -> str:
...
def _cmd_create_agent(project_id, args, ts) -> str:
    role = _find_role(args.get("role", ""))
...
def _cmd_create_team(project_id, args, ts) -> str:
...
def _cmd_build_team(project_id, args, ts) -> str:
...
def _cmd_hire_agent(project_id, args, ts) -> str:
    project = query_one("SELECT * FROM projects WHERE id = ?", (project_id,))
...
def _cmd_add_agent_to_team(project_id, args, ts) -> str:
    project = query_one("SELECT * FROM projects WHERE id = ?", (project_id,))
...
def _cmd_add_agent_from_other_team(project_id, args, ts) -> str:
    project = query_one("SELECT * FROM projects WHERE id = ?", (project_id,))
    agent = _find_agent_strict(args.get("agent"))
    if not agent:
...
def _cmd_remove_agent(project_id, args, ts) -> str:
    project = query_one("SELECT * FROM projects WHERE id = ?", (project_id,))
    agent = _find_agent_strict(args.get("agent"))
    if not agent:
        return f"Agent **{args.get('agent')}** not found (use the agent's name)."
...
def _cmd_install_toolchain(project_id, args, ts) -> str:
...
def _cmd_add_backlog_item(project_id, args, ts) -> str:
    bid = new_id()
...
def _cmd_create_sprint(project_id, args, ts) -> str:
...
def _cmd_add_task(project_id, args, ts) -> str:
...
def _cmd_add_urgent_task(project_id, args, ts) -> str:
    title = str(args.get("title") or args.get("task") or args.get("name") or "").strip()
    if not title:
...
def _cmd_align_team(project_id, args, ts) -> str:
    team = None
...
def _cmd_assign_task(project_id, args, ts) -> str:
...
def _cmd_start_task(project_id, args, ts) -> str:
    task = _find_task(project_id, args.get("task", ""))
    if not task:
...
def _cmd_start_sprint(project_id, args, ts) -> str:
...
def _cmd_resume_execution(project_id, args, ts) -> str:
...
def _cmd_cancel_execution(project_id, args, ts) -> str:
...
def _cmd_retry_failed(project_id, args, ts) -> str:
    failed = query(
...
def _cmd_enable_po(project_id, args, ts) -> str:
...
def _cmd_disable_po(project_id, args, ts) -> str:
...
_COMMAND_HANDLERS = {
    ...
    "create_skill": _cmd_create_skill,
    "create_agent": _cmd_create_agent,
    "create_team": _cmd_create_team,
    "build_team": _cmd_build_team,
    "hire_agent": _cmd_hire_agent,
    "add_agent_to_team": _cmd_add_agent_to_team,
    "add_agent_from_other_team": _cmd_add_agent_from_other_team,
    "remove_agent": _cmd_remove_agent,
    "install_toolchain": _cmd_install_toolchain,
    "add_backlog_item": _cmd_add_backlog_item,
    "create_sprint": _cmd_create_sprint,
    "add_task": _cmd_add_task,
    "add_urgent_task": _cmd_add_urgent_task,
    "align_team": _cmd_align_team,
    "assign_task": _cmd_assign_task,
    "start_task": _cmd_start_task,
    "start_sprint": _cmd_start_sprint,
    "stop_sprint": _cmd_stop_sprint,
    "pause_execution": _cmd_pause_execution,
    "resume_execution": _cmd_resume_execution,
    "cancel_execution": _cmd_cancel_execution,
...
def _execute_command(project_id, command, args) -> str:
    ...
    return handler(project_id, args, now())
...
def _extract_json(text: str):
    ...
    for candidate in candidates:
        for repaired, fixed in ((False, candidate), (True, _repair_json(candidate))):
            if not fixed:
                continue
            try:
                ...
                continue
            # A repaired or truncated response may be missing command args.
            # It can be shown as text, but must never execute a partial action.
            if _valid_ai_response(data) and (not repaired or data["command"] is None):
                return data
    return None


def _valid_ai_response(data) -> bool:
    """Reject repaired fragments that could execute a partial command."""
    ...
    return isinstance(suggestions, list) and all(
...
def _ai_route(project_id: str, conversation_id: str, text: str) -> dict:
    ...
    return _ai_route_response(project_id, conversation_id, parsed)


def _ai_route_response(project_id: str, conversation_id: str, parsed: dict) -> dict:
...
def _confirm_pending(project_id: str, conversation_id: str) -> dict:
    """Claim and execute one pending command under its conversation lock."""
    with _pending_lock(conversation_id):
        pending = query_one(
        ...
        except Exception as exc:  # keep pending so the user can retry
...
def _prepare_message(conversation_id: str, text: str) -> str:
    text = text.strip()
...
async def handle_message_async(project_id: str, conversation_id: str, text: str) -> dict:
    """Use async HTTP and backoff for open-ended chat; typed commands stay sync."""
    matched = any(re.match(pattern, text.strip(), re.IGNORECASE) for _, pattern in INTENTS)
    gw, model = await asyncio.to_thread(_bot_config)
    if matched or not gw:
        return await asyncio.to_thread(handle_message, project_id, conversation_id, text)
    text = await asyncio.to_thread(_prepare_message, conversation_id, text)
    context = await asyncio.to_thread(_context_brief, project_id)
    from .codegen import GatewayClient
    parsed = None
    last_exc = None
    for _ in range(2):
        try:
            raw = await GatewayClient.acall(gw, model, text, AI_SYSTEM_PROMPT + context,
                                            max_tokens=1500)
            parsed = _extract_json(raw["text"])
            if parsed and (parsed.get("reply") or parsed.get("command")):
                break
            raise RuntimeError("The model did not return a valid response.")
        except (RuntimeError, ValueError, KeyError, TypeError) as exc:
            last_exc = exc
    if parsed is None or not (parsed.get("reply") or parsed.get("command")):
        reply = (f"AI routing failed — falling back to typed commands. ({last_exc})\n\n"
                 "Try `help` for the command catalog.")
        await asyncio.to_thread(_save_message, conversation_id, "assistant", reply)
        return {"reply": reply, "suggestions": ["Status", "Help"]}
    return await asyncio.to_thread(_ai_route_response, project_id, conversation_id, parsed)
